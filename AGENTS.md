# dsh-calender —— DSH Web GUI 日历待办插件
...
- 卸载: `dsh plugin --profile web remove dsh-calender`
...
- **禁止做**:
  ...
  - 不要在浏览器端直接修改任务数据——一切经 `POST /api/calender/action` 提交，以 Host snapshot 为准
  - 不要用 `process.cwd()` 存放数据——账本固定 `$DSH_HOME/calender/ledger-v1.json`
...
- 客户端 `apply(ctx)` 内**只能**使用 `inject` 声明的服务 + 纯 DOM/transport；任何运行时可变的 DSH 域数据（会话标题、工作区、模型目录等）一律由 Host 半边经 `/api/calender/*` 读取，浏览器只 fetch
...
- `scripts/dsh-calender.js`: mount/unmount/status 辅助 CLI
...
- 真实组合：`dsh plugin --profile <scratch> add link:<本目录>` → `dsh --profile <scratch> --dump-config` 断言出现 `ui-calender` 层
...
- `/api/calender/*` 仅限 DSH loopback 同源 + 严格校验；POST 仅 JSON、载荷上限；禁止 shell 命令/可执行路径
