...
- **调度安全与可观测性**：定时 Agent 可以创建普通待办，但 Host 会拒绝它再次创建/武装自动触发 schedule，阻断递归；勾选 `triggerAgent` 时，匹配的模板首日和后续副本都会触发，留空 `triggerAt` 使用任务块开始时间；错过的一次性 dueAt 或重复首日记录为失败，不补跑。重复规则只物化当前/未来实例，错过的实例不补跑。执行 session 在 prompt 前写入 ledger，失败/重试会显示在任务状态与执行记录中。
