...
- **拖动重新定时**：移动带自动触发语义的任务会提交 Host `reschedule`，把这次移动明确视为新的触发 occurrence；已有成功或失败的 scheduled execution 只保留为历史，不会挡住同日的新时间。拖到过去只移动日历块，不补跑、不新增失败记录；重复规则的固定 `triggerAt` 与任务块时间解耦，改块只改变对应日期，空白 `triggerAt` 才跟随块开始。重复任务选择「改所有副本」时只改未来副本，过去副本和历史执行不重写；Host 快照中的 schedule、`nextRunAt`、重试计数和 executions 是展示与运行的同一来源。
