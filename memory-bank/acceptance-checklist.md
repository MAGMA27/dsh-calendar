...
- [x] 周视图：任务块**移动 / 拉伸**调时间；默认按 **15 分钟**吸附；顶部 sticky 表头（周几+日期）；**重叠任务并排**不遮盖；跨日任务的剩余部分从次日顶部显示
...
## 5. M6 完善
- [x] 设置里出现「插件」区 **calendar 设置卡**（announceToAgent / enabled 两个开关）
- [x] 关闭 announceToAgent 后 **SystemPrompt 段实时消失**（无需重启）
- [x] agent system prompt 能看到 plugin:calendar 宣告段
- [x] `node scripts/dsh-calendar.js status|mount|unmount` 可用
- [x] README / DESIGN / 进度 / 架构文档与当前实现一致
...
- [x] **副本与原任务保持同步**：改模板的标题/描述/Prompt/象限/执行钉子/子任务结构 → 所有副本跟着更新；各副本的完成态与执行记录独立；改单个副本只影响它自己
...
- [x] **触发 Agent 开关联动**：在副本上关掉/改「触发时间」→ 已物化未来副本的 🕐/到时随之更新；已过去的副本不受影响
- [x] 副本时间（拖拽移动/缩放）改动**必弹确认**：「只改这一个并解绑」（↻ 消失、独立于模板）或「改所有副本」（模板+全部副本同步平移）；**模板**拖拽同样弹确认（同步所有副本 / 只改模板）
- [x] **触发 Agent**：勾选后每个副本带 🕐，到点（默认任务时间段开始，可设「触发时间」覆盖）自动执行并结算、跑完该副本调度清除；不勾则副本只是日历条目
- [x] **跳过节假日**生效：周末 + 中国法定节假日（内置 2025 官方 / 2026 预估）不物化
- [x] 删除某一天的副本后**不会被重新补回**；删除重复**模板**级联删除仍绑定的副本（解绑副本保留）
