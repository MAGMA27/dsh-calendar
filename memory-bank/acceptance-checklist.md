...
- [x] `pnpm test` 全绿 = **168 单测**（vitest run，24 个测试文件）
...
- [ ] **无自由 cron 输入**：定时面板只有「不重复 / 每日 / 每周」+ 周几多选 + 「跳过周末与节假日」开关（+ 不重复模式的一次到时）
- [ ] 设了重复后，任务被**拷贝到各匹配日期**（副本是普通任务，↻ 徽标标记；模板当天不产生副本）
- [ ] 副本时间（拖拽移动/缩放）改动**必弹确认**：「只改这一个并解绑」（↻ 消失、独立于模板）或「改所有副本」（模板+全部副本同步平移）
- [ ] **跳过节假日**生效：周末 + 中国法定节假日（内置 2025 官方 / 2026 预估）不物化
- [ ] 删除某一天的副本后**不会被重新补回**；删除重复**模板**级联删除仍绑定的副本（解绑副本保留）
- [ ] 重复模板**不自动执行**（只是物化副本；副本可手动「立即执行」）
