# dsh-calendar 技术架构（Architecture）
...
浏览器（同源异步视图）                Host（权威）
┌──────────────────────────┐   HTTP   ┌──────────────────────────────┐
│ React 视图（周网格/矩阵/…） │ ────────▶ │ host-routes: /api/calendar/*   │
│ HttpHostTransport        │ ◀──────── │  state(GET) events(SSE) action(POST) │
│ sidebar-entry/mount      │  snapshot │ HostcalendarService            │
│ 设置卡 / locales          │          │  ├ host-ledger  ($DSH_HOME/calendar/ledger-v1.json, 原子+锁+幂等) │
└──────────────────────────┘          │  ├ host-service (cron tick 30s + 会话轮询 5s + 对账 + SSE) │
                                      │  ├ host-runner  (ApiProxy: 建会话→钉子→prompt→结算) │
                                      │  └ SystemPrompt.section(plugin:calendar) │
                                      └──────────────────────────────┘
```
...
- `GET /api/calendar/state`（no-store）→ `Snapshot{schemaVersion,revision,tasks,scheduler}`
- `GET /api/calendar/events`（SSE）→ revision/scheduler 变更提示（断线重连+页面恢复可见重拉）
- `GET /api/calendar/options` → 执行设置目录（工作区/会话(含标题,归档已滤)/provider+模型）
- `POST /api/calendar/action` → `{requestId, action}` → Snapshot；action：create/update/delete/archive/restore/setSchedule/run/import（`setSchedule` 的 cron/dueAt 用 `null` 清除、`undefined` 不动）
- 安全：loopback 同源 + 严格校验；POST 仅 JSON；普通 ≤64KiB / import ≤2MiB；action 无命令/可执行路径/shell 文本

## 5. 命名约定

包 `dsh-calendar` / 行 id `ui-calendar` / 设置命名空间 `calendar` / 存储键 `dsh.calendar.v1`（localStorage 迁移源）/ 账本 `$DSH_HOME/calendar/ledger-v1.json` / DOM `data-dsh-calendar-*` / 面板事件名 `calendar`。
