...
### 建任务起点改为向上取整（复测反馈）
按用户要求：`normalizeDrag` 的**起点**从「就近取整」改为 **`snapCeil`（向上取整）**——新建任务从不会早于被吸附的边界、不会回退进上一个格子；**终点仍就近取整**；`hi<=lo` 的退化区间强制为起点起的一个格（保证 start<end，且子格拖拽仍得合法一格）。新增「起点向上」「子格拖拽」用例（+2，202 全绿）。`alignCreateStart` 继续兜底把吸附进既有任务的起点推到其下缘。