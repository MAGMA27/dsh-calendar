...
### 建任务起点向下取整·endDrag 确定性收口（复测反馈·最终定稿）
按用户明确取值最终定稿：`normalizeDrag` **两端都 `snapFloor`（向下取整）**——起点 9:50→9:30、10:20→10:00、10:35→10:30；终点同向下取整。**两个真凶都处理**：① 此前为了让「贴着上个任务下缘新建」而新增的 `alignCreateStart` 会把起点**自动前推到某任务下缘**，导致漂移——**已彻底移除**该函数及其调用/用例；② `endDrag` 原先读可能过期的 `drag` state，且**纯点击（无 move）时起点是 startDrag 的原始值、根本没经过取整**——改为用 `dragOrigin`/`dragLast` 两个 ref 在**结束时确定性重算**并走 `normalizeDrag`，点击与拖动都统一向下取整。新增两条真实指针链路测试（stub grid rect：拖选 9:50→10:20 得 9:30–10:00、纯点击 9:50 得 9:30），合计 **200 全绿**。