...
### 建任务起点向下取整（复测反馈·定稿）
按用户明确取值定稿：`normalizeDrag` **两端都 `snapFloor`（向下取整）**——起点 9:50→9:30、10:20→10:00、10:35→10:30；**终点也向下取整，不再动它**。退化区间（hi<=lo）强制为起点起一格，保证 start<end 且子格拖拽得合法一段。测试覆盖三例起点 + 终点 floor + 反向拖拽 + 子格（合计 202 全绿）。`alignCreateStart` 照旧兜底：起点若被吸进既有任务块，推到其真实下缘，从而仍可贴着上个任务下缘新建。