...
- resize 按网格的绝对时间计算，向下拖到当天末尾可延伸到次日凌晨；跨日段按日期切片显示。
...
### 触发徽标语义修正（复测反馈）
问题：🕐 时钟徽标此前只要 `schedule.enabled === true` 就显示，导致未勾选「到点触发 Agent」的重复**模板**（为启用重复规则而 enabled）也带时钟图标，误导用户以为会触发。
修复：新增 `core/tasks.ts` 的 `taskTriggersAgent(task)` 作为时钟徽标唯一判定——仅当任务**确实会到期触发 agent**才显示：一次性 dueAt 到此自动执行，或重复规则的 `triggerAgent === true`；普通任务、未触发 Agent 的重复模板/副本、纯物化副本一律不显示。TaskBlock/TaskExtras 改用它，并把 tooltip 文案改为「到点触发 Agent / agent trigger at due time」。
...
### 任务详情时间段编辑（2026-08-20）
- 详情面板新增开始日期、固定 15 分钟时间选项和持续时间；持续时间失焦时吸附到最近的 15 分钟档位并限制在 15 分钟至 24 小时；普通任务保存时由 `startAt + duration` 计算并提交 `endAt`，可以直接从本周移动到下周并支持跨日。
- 重复系列通过已有的重复时间确认流程处理，避免详情编辑静默改动整个系列。
- 新增跨周保存与非法时间段 UI 回归测试；当前验证基线为 **218 单测全绿**（24 文件），`pnpm typecheck` / `pnpm build` 已通过。
### 周视图默认粒度统一（2026-08-20）
- `WeekGrid` 默认拖选、移动和拉伸吸附改为 15 分钟，与详情页的开始时间/持续时间设置一致；仍可通过 `snapMinutes` 传入其他粒度。
- 更新周视图拖选回归断言；当前验证基线仍为 **218 单测全绿**（24 文件）。
### 周视图跨日拖拽切片修复（2026-08-20）
- 修复时间网格底部坐标折回 00:00 的问题；任务移动/拉伸以绝对时间计算，向下拖到当天末尾后可延伸到次日凌晨。
- 周视图任务块和移动预览按日期切片，跨日任务在次日 00:00 顶部显示剩余部分。
- 新增跨日移动与跨日显示回归测试；当前验证基线为 **218 单测全绿**（24 文件）。
### 拖选建任务贴合上个任务下边缘（复测反馈）
