...
4. **月视图日期栏色带（commit `a003320`→`ea5a05c`，用户复测追加）**：每格日期行加颜色条；复测迭代——3px 细线→胶囊→横贯整格横条→**贴住格子上边缘（margin 抵消 padding，与 grid 边框融为一体）**。关键修复：**颜色不能用 `--dsw-alias-brand-primary`（亮色下映射近黑 bluish-1000，显示成黑色），改用品牌蓝 `--dsw-static-deepseek-500`**，文字用白色 `--dsw-static-neutral-bluish-50`；相邻月灰色、今天 450 亮蓝 + 加粗。改动：`MonthGrid.tsx`、`calendar.module.css`。
