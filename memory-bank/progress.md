...
### 建任务起点向下取整·真根因是表头偏移（复测反馈·最终定稿）
按用户明确取值最终定稿：`normalizeDrag` **两端都 `snapFloor`（向下取整）**——起点 9:50→9:30、10:20→10:00、10:35→10:30；终点同向下取整。**真正的根因终于定位**：`WeekGrid` 的 `yToMs`/`yToMinutes`/`xToDayIndex` 原来用的是 **`bodyRef`（整个 weekGrid，含顶部 sticky 星期表头）** 的 rect，而小时线是相对**不含表头的 `weekGridCells`** 定位的——表头约 30–40px 使每次点击都被算得**晚了约 30 分钟**（视觉 9:50 → 代码 ~10:13 → floor 10:00），这解释了「点 9:50 却是 10:00」跨多轮取整方向都不变的现象。**修复**：新增 `cellsRef` 指向 `weekGridCells`，时间/坐标换算全部改用它（排除表头）。另外两处已处理：移除会前推起点的 `alignCreateStart`；`endDrag` 用 ref 确定性重算、纯点击也走 floor。测试 stub 改为带 40px 表头偏移的网格 rect，验证排除表头后 9:50→9:30（合计 200 全绿）。