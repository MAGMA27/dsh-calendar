...
4. **月视图日期栏色带（commit `a003320`→`c3d0b63`，用户复测追加）**：每格日期行加颜色条；复测后从「3px 细线」改为「胶囊色带」——日期数字直接浮在色带上：当前月品牌色、相邻月灰色（75%）、今天加品牌色外环。改动：`MonthGrid.tsx`（`.monthCellDateRow` 即色带、数字浮其上）、`calendar.module.css`。
