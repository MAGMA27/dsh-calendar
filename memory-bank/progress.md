...
4. **月视图日期栏色带（commit `a003320`→`1659aa1`，用户复测追加）**：每格日期行加颜色条；复测迭代——先 3px 细线→改胶囊色带→最终**横穿整格宽度的圆角横条**、日期数字浮在其上：当前月品牌色、相邻月灰色（75%）、今天加品牌色外环。改动：`MonthGrid.tsx`（`.monthCellDateRow` 即色带、数字浮其上）、`calendar.module.css`。
