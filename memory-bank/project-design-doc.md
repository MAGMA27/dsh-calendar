...
| 艾森豪威尔 | 紧急/重要各三级 → 四象限配色 | ✅ M2（展示）；🔜 象限拖拽改优先级 M3 |
| 子任务 | 主任务挂子任务清单（勾选/进度/折叠） | 🔜 M3 |
| 执行会话标记 | 任务钉住工作区 + 执行会话（新建/复用） | 🔜 M3 |
...
| 任务详情面板 | 右侧 320px：编辑/子任务/执行设置/定时/执行记录 | 🔜 M3 |
...
- 主体：M2 = 整块日历区（浮层弹窗）；M3 = 左日历 + 右 320px 详情面板（窄屏 overlay）。
...
### 4.4 组件细节
- 任务块徽标（M2）：`✓ done/total`、`provider·model`、定时钟。
...
- CreateTaskModal（M2）：预填起止+标题+紧急/重要；M3 扩为完整表单。
