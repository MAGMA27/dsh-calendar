...
function emptySnap(): CalenderSnapshot {
...
function makeSnapTransport() {
  let snap = emptySnap()
  const applier = (a: CalenderAction): CalenderSnapshot => {
    if (a.kind === 'create') {
  ...
  const transport = new MemoryCalenderHostTransport(snap, applier)
  return { transport, getSnap: () => snap }
}

describe('CalenderClientController', () => {
  it('starts and transitions to ready', async () => {
    const { transport } = makeSnapTransport()
    const c = new CalenderClientController(transport, initialState(0, 0))
    await c.start()
