...
import { HttpCalenderHostTransport } from '../src/client/host-api.ts'
import type { CalenderSnapshot } from '../src/protocol.ts'

function snap(): CalenderSnapshot {
  return { schemaVersion: 1, revision: 3, tasks: [], scheduler: { ledgerId: 'L', timeZone: 'z' } }
}

describe('HttpCalenderHostTransport', () => {
  it('reads state via GET /api/calender/state', async () => {
    const patch = vi.fn().mockResolvedValue(new Response(JSON.stringify(snap()), { status: 200 }))
    vi.stubGlobal('fetch', patch)
    const t = new HttpCalenderHostTransport('/api/calender', { getItem: () => null, setItem: () => {} })
    const s = await t.state()
    expect(s.revision).toBe(3)
    expect(patch).toHaveBeenCalledWith('/api/calender/state', expect.objectContaining({ cache: 'no-store' }))
  })
  it('submits an action envelope to POST /api/calender/action', async () => {
    const patch = vi.fn().mockResolvedValue(new Response(JSON.stringify(snap()), { status: 200 }))
    vi.stubGlobal('fetch', patch)
    const t = new HttpCalenderHostTransport('/api/calender', { getItem: () => null, setItem: () => {} }, () => 'rid-1')
    ...
    expect(url).toBe('/api/calender/action')
  ...
  it('bootstrap imports legacy tasks once per ledger and marks the marker', async () => {
    ...
    vi.stubGlobal('fetch', patch)
    const t = new HttpCalenderHostTransport('/api/calender', storage as any, () => 'r')
    ...
    expect(storage.setItem).toHaveBeenCalledWith('dsh.calender.v1.sourceId', 'r')
    expect(storage.setItem).toHaveBeenCalledWith('dsh.calender.v1.hostImported', 'L')
  })
})
